 ...
 *
 * <p>
 * A {@code ReentrantLock} is <em>owned</em> by the thread last successfully locking, but not yet unlocking it. A thread
 * invoking {@code lock} will return, successfully acquiring the lock, when the lock is not owned by another thread. The
 * method will return immediately if the current thread already owns the lock. This can be checked using methods
 ...
 * (i.e., are slower; often much slower) than those using the default setting, but have smaller variances in times to
 * obtain locks and guarantee lack of starvation. Note however, that fairness of locks does not guarantee fairness of
 * thread scheduling. Thus, one of many threads using a fair lock may obtain it multiple times in succession while other
 * active threads are not progressing and not currently holding the lock. Also note that the untimed {@link #tryLock()}
 * method does not honor the fairness setting. It will succeed if the lock is available even if other threads are
 * waiting.
 *
 * <p>
...
public class ReentrantLock implements Lock, java.io.Serializable {
     ...
     * lies dormant until the lock has been acquired, at which time the lock hold count is set to one.
     */
    public void lock() {
        sync.lock();
    }
    ...
    /**
     * Acquires the lock if it is not held by another thread within the given waiting time and the current thread has
     * not been {@linkplain Thread#interrupt interrupted}.
     *
     * <p>
     * Acquires the lock if it is not held by another thread and returns immediately with the value {@code true},
     * setting the lock hold count to one. If this lock has been set to use a fair ordering policy then an available
     * lock <em>will not</em> be acquired if any other threads are waiting for the lock. This is in contrast to the
     ...
     * <p>
     * In this implementation, as this method is an explicit interruption point, preference is given to responding to
     * the interrupt over normal or reentrant acquisition of the lock, and over reporting the elapse of the waiting
     * time.
     *
     ...
     * <ul>
     *
     * <li>If this lock is not held when any of the {@link Condition} {@linkplain Condition#await() waiting} or
     * {@linkplain Condition#signal signalling} methods are called, then an {@link IllegalMonitorStateException} is
     * thrown.
     *
     * <li>When the condition {@linkplain Condition#await() waiting} methods are called the lock is released and, before
    ...
    /**
     * Queries if this lock is held by any thread. This method is designed for use in monitoring of the system state,
     ...
     * example, the owner may be momentarily {@code null} even if there are threads trying to acquire the lock but have
     * not yet done so. This method is designed to facilitate construction of subclasses that provide more extensive
    ...
    /**
     * Returns an estimate of the number of threads waiting to acquire this lock. The value is only an estimate because
     * the number of threads may change dynamically while this method traverses internal data structures. This method is
     ...
     * because timeouts and interrupts may occur at any time, the estimate serves only as an upper bound on the actual
     * number of waiters. This method is designed for use in monitoring of the system state, not for synchronization
     * control.
     *
     * @param condition the condition
     ...
     * lock. Because the actual set of threads may change dynamically while constructing this result, the returned
     ...
     * String {@code "Unlocked"} or the String {@code "Locked by"} followed by the {@linkplain Thread#getName name} of
     * the owning thread.
     *
